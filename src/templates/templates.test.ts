...
describe("generateBootstrapPrompt", () => {
  test("includes agent name", () => {
    ...
    expect(prompt).toContain("Your name is Klaus.");
  });
  ...
  test("includes agent context when provided", () => {
    const prompt = generateBootstrapPrompt({
      agent: {
        name: "Klaus",
        context:
          "A family agent — part AI assistant, part Berlin raccoon. Named after Claude, but more German. 🦝",
      },
    });
    expect(prompt).toContain("Berlin raccoon");
    expect(prompt).toContain("Named after Claude");
  ...
  test("includes user name and context when provided", () => {
    const prompt = generateBootstrapPrompt({
      ...
      user: {
        name: "Tim",
        context: "Lives in Berlin with Tati and Lily. Timezone Europe/Berlin.",
      },
    });
    expect(prompt).toContain("Your user is Tim.");
    expect(prompt).toContain("Lives in Berlin");
    expect(prompt).toContain("Tati and Lily");
  });
